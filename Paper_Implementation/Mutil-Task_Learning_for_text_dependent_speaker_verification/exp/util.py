...
def get_argparser():
    ...
    parser.add_argument('--n_class_task1', type=int, default=10)
    parser.add_argument('--n_class_task2', type=int, default=60)
    parser.add_argument('--batch_size_train', type=int, default=32)
    parser.add_argument('--batch_size_test', type=int, default=128)
    parser.add_argument('--target_length', type=int, default=21000)
    #parser.add_argument('--base_dir', type=str, required=True)
    parser.add_argument('--base_dir', type=str, default='E:/정리/data/audio_mnist')
    #....
...
def mfcc_read_srp(mfcc_scp):
    key2mfcc = {}
    with open(mfcc_scp, "r") as f :
        for line in f.readlines():
            key, path = line.strip().split()
            key2mfcc[key] = np.load(path)
