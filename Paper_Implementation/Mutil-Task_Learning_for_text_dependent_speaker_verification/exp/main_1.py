...
from lib.util import get_argparser, load_dataset, set_length, preprocess_dataset
from lib.evaluate import predict_eval_module
from lib.dataset import CustomDataset
from lib.network import CNNclassification
from lib.train import train
import argparse
from lib.resnet import ResNet ,resnet18
...
def main(args: argparse.Namespace) -> None:
    ...
    train_scp = f'{args.base_dir}/wav_audio_mnist_train.scp' # 경로 이 부분 배웠다. 다시 복습
    dev_scp = f'{args.base_dir}/wav_audio_mnist_dev.scp' # 경로 이 부분 배웠다. 다시 복습
    eval_scp = f'{args.base_dir}/wav_audio_mnist_eval.scp'# 경로 이 부분 배웠다. 다시 복습
    ...
    for epoch in range(1, args.n_epochs + 1):
        train(model, optimizer, train_loader, scheduler, device, epoch) # , device
